...
def test_graph_rag_ingest(monkeypatch):
    ...
    monkeypatch.setattr(gr, 'GraphDatabase', type('GD', (), {'driver': lambda *a, **k: DummyDriver(*a, **k)}))

    # construct GraphRAG and ingest
    g = GraphRAG(uri='neo4j://fake:7687', user='u', password='p')
    ok = g.ingest([{'videoId':'v1','title':'t','description':'d','channelTitle':'c','publishedAt':'2025-01-01','viewCount':10,'durationSeconds':60,'suggestedTags':['tag1']}])
    g.close()
    assert ok
...
def test_youtube_uses_graph_rag(monkeypatch):
    ...
    records = [{'videoId':'v1','title':'t','description':'d','channelTitle':'c','publishedAt':'2025-01-01','viewCount':10,'durationSeconds':60,'suggestedTags':['tag1']}]
